...
if(prep.corpusCommit!==inventory.corpusCommit||prep.registros.length!==0||new Set(prep.registros.map(x=>x.id)).size!==0)errors.push('preparação incompleta');
...
if(errors.length){console.error(errors.join('\n'));process.exit(1)}console.log('SRD2: fila de adversários adicionais concluída.');
