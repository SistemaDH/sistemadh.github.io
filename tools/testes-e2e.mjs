...
try {
  ...
  await passo('receber dano usa os limiares da ficha sem rolar dados', async () => {
  ...
  await passo('comunidades ativas chegam à ficha: Mochila Nômade e Conhece a Maré', async () => {
    await pagina.locator('.ficha__topo button[aria-label="Voltar para a lista"]').click();
    await pagina.waitForSelector('.ficha-cartao__abrir');
    const def = noBackend('ABAS.PERSONAGENS');
    const linhas = ambiente.contexto.lerTudo_(def)
    ...
    try {
      ...
      const usar = dobraCaracs.getByRole('button', { name: 'Vasculhar Mochila Nômade · 1 Esperança' });
  ...
  await passo('classe e subclasse abrem o que está atrás delas (ponto 4)', async () => {
    ...
    const arteSubclasse = visor.locator('.carta-visor__img').first();
  ...
  await passo('a mochila registra e o gerenciador equipa uma arma possuída', async () => {
