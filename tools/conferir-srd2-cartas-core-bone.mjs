...
const auditoria = ler('data/srd2-cartas-core-bone-auditoria.json');
...
const cartas = new Map(catalogo.cartas.filter((carta) => carta.dominio === 'BONE').map((carta) => [carta.id, carta]));

if (auditoria.estado !== 'conferido') erros.push('auditoria de Osso ainda não está conferida');
if (auditoria.dominio !== 'domains/bone') erros.push('domínio da auditoria deve ser Osso');
...
if (cartas.size !== 21) erros.push(`catálogo local tem ${cartas.size} cartas de Osso, esperado 21`);
...
const textosAtivos = catalogo.cartas.filter((carta) => carta.dominio === 'BONE').map((carta) => JSON.stringify({
  texto: carta.texto,
...
const termosLegados = [
  /\batributo(?: de)? (?:Agilidade|Conhecimento)\b/i,
  /\brolagem de dano\b/i,
  /\bEspaços? de Armadura\b/i,
  /\bcartas de domínio no (?:seu )?loadout\b/i,
  /\bloadout\b/i,
  /\balcance Longo\b/i,
  /\bGM\b/,
  /\bJogada de (?:Agilidade|Instinto)\b/,
  /\b(?:um Estresse|uma Esperança|[1-9] (?:ponto )?de (?:Estresse|Esperança))\b/i,
  /\bDestreza ou Agilidade\b/i
];
for (const termo of termosLegados) {
  if (termo.test(textosAtivos)) erros.push(`vocabulário mecânico legado ainda ativo em Osso: ${termo}`);
}
...
console.log('SRD2: 21 cartas de Osso conferidas em nível, tipo, custo de recordar, mecânica, erratas e vocabulário pt-BR.');
