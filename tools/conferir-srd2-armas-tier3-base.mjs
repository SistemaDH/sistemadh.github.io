...
const audit=read('data/srd2-armas-tier3-base-auditoria.json');
...
for(const [src,id,damage] of audit.registros){
 ...
 if(record?.estado!=='mecanica-implementada'||![60,61,62,68].includes(record.sourceLocator?.pdfPageStart)||!item||item.tier!==3||item.danoSRD!==damage||item.nomeIngles!==record.nomeIngles)errors.push(src+': vínculo divergente');
}
if(audit.registros.length!==18||new Set(audit.registros.map(x=>x[0])).size!==18||errors.length){console.error(errors.join('\n'));process.exit(1)}
console.log('SRD2: 18 versões avançadas do 3º patamar vinculadas ao inventário e catálogo.');
