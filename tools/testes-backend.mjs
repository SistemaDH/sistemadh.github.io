...
teste('as 129 fichas e os 19 ambientes estão no servidor', () => {
  igual(avaliar('ADVERSARIOS.length'), 163);
...
teste('o catálogo filtra por patamar, tipo e busca', () => {
  ...
  igual(t1.total, 163, 'o total é sempre o do catálogo inteiro');
...
teste('Dobradora da Sorte cobra 3 Esperanças, respeita 1/sessão e volta na próxima', () => {
  ...
  verdade(/Dados de Dualidade/.test(r.mudancas[0].aviso || ''), JSON.stringify(r.mudancas[0]));
...
teste('Passo Sombrio cobra 1 Estresse, liga Camuflado e publica alcance Distante', () => {
  ...
  igual(r.mudancas[0].alcance, 'Distante');
});

teste('Sombra Fugaz aumenta somente o alcance de Passo Sombrio para Longínquo', () => {
  const f = ladinoNoturnoLote8_(['fundacao', 'especializacao', 'maestria']);
  const evasao = f.defesas.evasao;
  const r = contexto.aplicarAjustes_(f, [{ tipo: 'habilidade', nome: 'Passo Sombrio' }]);
  igual(r.erros.length, 0, JSON.stringify(r));
  igual(r.mudancas[0].alcance, 'Longínquo');
